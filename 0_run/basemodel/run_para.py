import mph
import pandas as pd
import os
from multiprocessing import Pool
import numpy as np

absolutePath = os.getcwd()
basic_model_path = os.path.join(absolutePath, 'bubble_basic_model.mph')
filename = os.path.join(absolutePath, 'inputdata.csv')

core_num=2
para_num=1
t_step=0.002
t_max=500
Bos=[0.0002,0.0004,0.0007,0.001,0.002,0.004,0.007,0.01,0.02,0.04,0.07,0.4,0.6,0.8,1]
# Gas=[5.0,5.5,6.0,6.5,6.25,6.75,7.0,8.0,9.0,10,15,20,30,40,50,60,80,700,200,400,600,800,1000]
Gas=[6.0,6.5,7.0]
Ga_Bos=[]
for Bo in Bos:
    for Ga in Gas:
        Ga_Bos.append([Ga,Bo])

def run_simulation(params):
    Ga, Bo  = params
    L_rise=10
    refine_num=7
    # Connect to a COMSOL server instance
    client = mph.start(cores=core_num)  # Adjust the number of cores as needed
    saveFileName = os.path.join(absolutePath, f'bubble_L_rise{L_rise}_Ga{Ga}_Bo{Bo}_Level{refine_num}_base.mph')
    saveResultsName = os.path.join(absolutePath, f'results_L_rise{L_rise}_Ga{Ga}_Bo{Bo}_Level{refine_num}.csv')
    saveDimensionResultsName = os.path.join(absolutePath, f'results_dimension_L_rise{L_rise}_Ga{Ga}_Bo{Bo}_Level{refine_num}.csv')

    if os.path.exists(saveResultsName):
        print(f'Results already exist for L_rise = {L_rise}, Ga = {Ga}, Bo = {Bo}')
        return

    # Load the model
    model = client.load(basic_model_path)    

    # Set parameters
    model.parameter('L_rise', str(L_rise))
    model.parameter('Ga', str(Ga))
    model.parameter('Bo', str(Bo))
    model.parameter('t_max', str(t_max))
    model.parameter('t_step', str(t_step)) 
    model.parameter('refine', str(refine_num)) 
    model.save(saveFileName)
    # # Try to run the solution
    try:
        model.solve()

        column=['t','velocity','center','max_x','max_y','min_y','ratio','area','R_eq','E_k','E_P','E_e','E_s','Int_Ee']        
        results = model.evaluate(column, dataset='Study 1//Remeshed Solution 1')
        df = pd.DataFrame(results, columns=column)        
        df = df.round(5)
        min_EP = df['E_P'].min()
        df['E_P'] = df['E_P'] - min_EP
        df['center'] = -1*df['center']
        df['max_y'] = -1*df['max_y']
        df['min_y'] = -1*df['min_y']

        df.to_csv(saveResultsName, index=False)
        
        model.save(saveFileName)
        client.remove(model)
        print(f'Success for L_rise = {L_rise}, Ga = {Ga}, Bo = {Bo}\n')
    except Exception as e:
        column=['t','velocity','center','max_x','max_y','min_y','ratio','area','R_eq','E_k','E_P','E_e','E_s','Int_Ee']        
        results = model.evaluate(column, dataset='Study 1//Remeshed Solution 1')
        df = pd.DataFrame(results, columns=column)        
        df = df.round(5)
        min_EP = df['E_P'].min()
        df['E_P'] = df['E_P'] - min_EP
        df['center'] = -1*df['center']
        df['max_y'] = -1*df['max_y']
        df['min_y'] = -1*df['min_y']

        df.to_csv(saveResultsName, index=False)
        model.save(saveFileName)
        client.remove(model)
        print(f'Error occurred for L_rise = {L_rise}, Ga = {Ga}, Bo = {Bo}:\n{e}')

def main():
    # Set the absolute path and load the data    
    # data = pd.read_csv(filename, skiprows=1, header=None)
    # params = zip(data.iloc[:, 0], data.iloc[:, 1], data.iloc[:, 2])

    # Use multiprocessing to run simulations in parallel
    with Pool(para_num) as pool:  # Adjust the number of parallel processes
        pool.map(run_simulation, Ga_Bos)

if __name__ == '__main__':
    main()
